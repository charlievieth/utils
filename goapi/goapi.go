...
import (
	...
	exec "github.com/charlievieth/utils/goapi/execabs"
)
...
var (
	...
	rootDirs   = flag.String("root", "", "optional comma-separated list of root directories to scan")
	goPkgs     = flag.String("pkgs", "", "optional comma-separated list of packages to scan")
)
...
func copyContext(ctxt *build.Context) *build.Context {
	dupe := *ctxt
	return &dupe
}
...
func main() {
	...
	var updateDir bool
	if *rootDirs != "" {
		roots = strings.Split(*rootDirs, ",")
		for _, root := range roots {
			pkg, err := build.ImportDir(root, build.ImportComment)
			if err != nil {
	...
	walkers := make([]*Walker, len(contexts)*len(roots))
	...
	for _, context := range contexts {
		for j, root := range roots {
			...
			if updateDir {
				context.Dir = root
				pkgName = pkgs[j].ImportPath
			}
			// fmt.Fprintf(os.Stderr, "root: %s\n", root)
			go func(context *build.Context, root, pkgName string, i int) {
				defer wg.Done()
				walkers[i] = NewWalker(context, root, pkgName)
			}(context, root, pkgName, i)
			i++
		}
		// i, context := i, context
		// wg.Add(1)
...
func NewWalker(context *build.Context, root, pkg string) *Walker {
	if pkg == "" {
		pkg = "std"
	}
	w := &Walker{
...
func (w *Walker) loadImports() {
	...
	if !ok {
		...
		// WARN: FIX ME
		cmd := exec.Command(goCmd(), "list", "-e", "-deps", "-json", w.pkg)
