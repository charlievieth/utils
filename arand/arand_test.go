...
func TestUniformFactorial(t *testing.T) {
	r := rand.New(NewSource(testSeeds[0]))
	top := 6
	if testing.Short() {
		top = 3
	}
	for n := 3; n <= top; n++ {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			tests := [...]struct {
				...
				// WARN (CEV): we run "Int31n" twice because we cant run int31n() and it
...
func BenchmarkInt63UnthreadsafeParallel(b *testing.B) {
	r := rand.New(NewSource(1))
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			r.Int63()
